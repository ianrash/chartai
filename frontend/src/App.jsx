              ...
              <div className="flex flex-col gap-5 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <AnalysisCard
                    ...
                    sections={(() => {
                      ...
                      // Also try MTF order block for demand
                      if (!demand?.range && analysis.mtf_analysis?.order_block?.range) {
                        const isBullish = analysis.htf_analysis?.trend?.direction === "Bullish" || analysis.overall_trend === "Bullish";
                        if (isBullish) {
                          demand = { range: analysis.mtf_analysis.order_block.range, status: analysis.mtf_analysis.order_block.status };
                        }
                      }
                      ...
                      // Also try HTF order block for supply
                      if (!supply?.range && analysis.htf_analysis?.order_block?.range) {
                        const isBearish = analysis.htf_analysis?.trend?.direction === "Bearish";
                        if (isBearish) {
                          supply = { range: analysis.htf_analysis.order_block.range, status: analysis.htf_analysis.order_block.status };
                        }
                      }
                      ...
                      if (!fvg?.range) {
                        if (analysis.htf_analysis?.fvg?.nearest_above || analysis.htf_analysis?.fvg?.nearest_below) {
                          ...
                          fvg = { 
                            range: `${analysis.mtf_analysis.fvg.nearest_below || '—'} - ${analysis.mtf_analysis.fvg.nearest_above || '—'}`,
                            direction: "Above/Below",
                ...
                {(() => {
                    ...
                    if (!tradeData) {
                      ...
                      const confScore = analysis.confidence_score || (rating === "A+" ? 95 : rating === "A" ? 85 : rating === "B" ? 70 : rating === "C" ? 50 : 20);
                      ...
                      if ((!tradeData?.bias || tradeData.bias === "WAIT") && analysis.executive_summary) {
                        ...
                        if (hasBuySignal || hasSellSignal) {
                          const isBuy = hasBuySignal && !hasSellSignal;
                          tradeData = {
                            bias: isBuy ? "BUY" : "SELL",
                            label: isBuy ? "Bullish Setup" : "Bearish Setup",
                            execution: {
                              entry_zone: htfOB || mtfOB || "Awaiting confirmation",
                              entry: "Market",
